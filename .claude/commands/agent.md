...
2. **워크스페이스 파일 복사**
   - 각 에이전트별 `~/.openclaw/workspace-{agent}/` 및 `skills/gog/` 디렉터리 생성
   - `config/workspace-{agent}/` → `~/.openclaw/workspace-{agent}/` 복사
   - 스킬 복사: `config/skills/gog/SKILL.md` → 각 워크스페이스 `skills/gog/SKILL.md`
   - 파일 없으면 `log_warn`
